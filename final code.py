...
# -------------------------------------------------------------------
# حقن كود تتبع Google Analytics لتحليل الزوار وتحسين محركات البحث
# -------------------------------------------------------------------
...
def load_css(file_name: str):
    """دالة لتحميل وتطبيق ملف CSS خارجي."""
    try:
        with open(file_name, encoding="utf-8") as f:
            st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)
    except FileNotFoundError:
        st.warning(f"ملف الأنماط {file_name} غير موجود.")

# تطبيق ملف التصميم المخصص
...
# --------------------------------
# تعريف الدوال الرياضية والمالية الأساسية
# --------------------------------
def fmt(x: float) -> str:
    """تنسيق الأرقام مع فواصل الآلاف وتقريب لمنزلتين عشريتين."""
    return f"{x:,.2f}"

def simple_interest(P: float, r: float, t_months: int) -> Tuple[float, float]:
    """حساب الفائدة البسيطة والمبلغ الإجمالي."""
...
def compound_interest(P: float, r: float, t_months: int, m: int) -> Tuple[float, float]:
    """حساب الفائدة المركبة والمبلغ الإجمالي."""
    t_years = t_months / 12.0
...
def continuous_compound(P: float, r: float, t_months: int) -> Tuple[float, float]:
    """حساب الفائدة المركبة المستمرة والمبلغ الإجمالي."""
    t_years = t_months / 12.0
...
def monthly_payment(P: float, r: float, t_months: int) -> float:
    """حساب القسط الشهري الثابت للقرض."""
...
def amortization_schedule(P: float, r: float, t_months: int) -> pd.DataFrame:
    ...
    # تقريب النتائج في الجدول
    for col in ["القسط الشهري", "الفائدة المدفوعة", "أصل الدين المسدد", "الرصيد المتبقي"]:
        df[col] = df[col].apply(lambda x: round(x, 2))
    return df

# --------------------------------
# إعداد بنية التحكم في واجهة المستخدم
# --------------------------------
# قاموس لتنظيم أنواع الحسابات وتسهيل إدارتها
CALCULATION_MODES = {
...
# --------------------------------
# منطق الحساب وعرض النتائج عند الضغط على الزر
# --------------------------------
if calculate_button:
    P = inputs["P"]
    r_pct = inputs["r_pct"]
    ...
    if not P or P <= 0 or not r_pct or r_pct < 0 or not t_months or t_months <= 0:
        ...
        # تنفيذ الحساب وعرض النتائج الرقمية
        result = func(**params)
        ...
        # عرض التحليل التفصيلي والرسوم البيانية للقروض
        ...
        # عرض الرسم البياني للنمو لأنواع الفائدة الأخرى
        else:
...
# تذييل الصفحة لعرض معلومات المقرر
st.markdown(
    """
    <style>
    .footer-academic {
