...
# --- التعديل هنا: إضافة أيقونة الصفحة (Favicon) وتغيير العنوان ---
st.set_page_config(
...
def fmt(x: float) -> str:
    """تنسيق الرقم كعملة مع فاصلة الآلاف وبدقة عشرية."""
    return f"{x:,.2f}"
...
CALCULATION_MODES = {
    "الفائدة البسيطة": {
        ...
        "params": ["P", "r", "t_months"],
        "result_labels": ("إجمالي الفائدة", "المبلغ النهائي")
    },
    ...
    "القسط الشهري للقرض (تحليل الاستهلاك)": {
...
# --- التعديل هنا: إضافة البنر وتغيير العنوان ---
# 1. إضافة البنر في أعلى الصفحة
try:
    st.image("banner.png", use_column_width=True)
except FileNotFoundError:
    st.error("ملف البنر 'banner.png' غير موجود. يرجى التأكد من وجوده في نفس المجلد.")

# 2. تغيير العنوان إلى st.header ليكون أصغر
st.header("حاسبة الفوائد والقسط الشهري")
# --- نهاية التعديل ---
...
def clear_fields():
    """إعادة تعيين قيم حقول الإدخال في حالة الجلسة."""
...
if calculate_button:
    ...
    if P is None or r_pct is None or t_months is None:
        st.error("يرجى إدخال جميع القيم المطلوبة.")
    elif P <= 0 or r_pct < 0 or t_months <= 0:
        st.error("يجب أن يكون المبلغ أكبر من صفر، والنسبة موجبة، والمدة أكبر من صفر.")
    else:
        r = r_pct / 100.0
...
with st.expander("المنهجية والمعادلات الرياضية المستخدمة"):
    ...
    **1. القسط الشهري للقرض (Amortization):**
    ...
    $$
    *   $M$: القسط الشهري, $P$: أصل القرض, $i$: سعر الفائدة الشهري, $n$: عدد الأشهر

    **2. الفائدة المركبة (Compound Interest):**
    $$
    A = P \\left(1 + \\frac{r}{m}\\right)^{mt}
    $$
    *   $A$: المبلغ النهائي, $P$: أصل المبلغ, $r$: سعر الفائدة السنوي, $m$: عدد الدفعات, $t$: المدة بالسنوات

    **3. الفائدة المركبة المستمرة (Continuous Compounding):**
    $$
    A = P e^{rt}
    $$
    *   $A$: المبلغ النهائي, $P$: أصل المبلغ, $e$: الثابت الرياضي, $r$: سعر الفائدة السنوي, $t$: المدة بالسنوات
